...
The `lazyman` Neovim configuration is installed as `~/.config/lazyman/Lazyman`
...
```bash
export NVIM_APPNAME="lazyman/LazyVim"
```
...
`$HOME/.config/lazyman/Lazyman/lazyman.sh` : initializes the `Lazyman` Neovim configuration in `$HOME/.config/lazyman/Lazyman/`
...
`lazyman -a` : installs and initializes the `AstroNvimPlus` Neovim configuration in `$HOME/.config/lazyman/AstroNvimPlus/`

`lazyman -c` : installs and initializes the `NvChad` Neovim configuration in `$HOME/.config/lazyman/NvChad/`

`lazyman -l` : installs and initializes the `LazyVim` Neovim configuration in `$HOME/.config/lazyman/LazyVim/`
...
`lazyman -U -N LazyVim` : updates the `LazyVim` Neovim configuration in `$HOME/.config/lazyman/LazyVim/`

`lazyman -A -U` : updates all installed supported Neovim configurations

`lazyman -P -C https://github.com/Abstract-IDE/Abstract -N Abstract` : installs and initializes the Packer based 'Abstract' Neovim configuration in `~/.config/lazyman/Abstract`

`lazyman -R -N LazyVim` : removes the `LazyVim` Neovim configuration in `$HOME/.config/lazyman/LazyVim/`, its data files, cache, state, and all backups
...
a richly preconfigured Neovim configuration in `~/.config/lazyman/Lazyman`.
...
quick and easy way to re-configure the `Lazyman` Neovim configuration but you
...
(`<leader>lm`). While in Neovim with the default `Lazyman` configuration,
...
command, the `Lazyman` Neovim configuration, the `nvims` shell function,
and the configured keymaps.

While in Neovim using the `Lazyman` configuration, view the `vimdoc`

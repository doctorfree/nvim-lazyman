...
usage() {
  printf "\nUsage: webdev_config [-d] [-i] [-m menu] [-u]"
  printf "\nWhere:"
...
show_plug_help() {
  if [ "${have_rich}" ]; then
    rich "[cyan]Webdev Plugins Menu Help[/cyan]" -p -a rounded -c -C
  else
    printf "\n\tWebdev Plugins Menu Help\n"
  fi
  ...
  printf "\nSettings in this menu only effect the Webdev Neovim configuration in:"
  printf "\n\t${HOME}/.config/nvim-Webdev\n"
  prompt_continue
}
...
show_plugin_menu() {
  ...
  while true; do
    ...
    [ "$debug" ] || tput reset
    if [ "${have_rich}" ]; then
      rich "[b cyan]Webdev Plugins Configuration Menu[/]" -p -a rounded -c -C
      rich "[b green]Manage the Neovim plugins enabled in[/] [b yellow]~/.config/nvim-Webdev[/]" -p -c
    else
        ...
        "GPT4ALL"*,* | *,"GPT4ALL"*)
          if [ "${enable_codeexplain}" == "true" ]; then
            ...
            if [ -f "${HOME}/.codeexplain/model.bin" ]; then
              ...
              [ -x ${WDEVDIR}/scripts/gpt4all.sh ] || {
                printf "\n${WDEVDIR}/scripts/gpt4all.sh not executable"
                printf "\nUnable to enable the codeexplain.nvim plugin."
                printf "\nPlease check the Webdev installation."
                prompt_continue
              }
...
show_lsp_help() {
  if [ "${have_rich}" ]; then
    rich "[cyan]Webdev LSP Servers Menu Help[/cyan]" -p -a rounded -c -C
  else
    printf "\n\tWebdev LSP Servers Menu Help\n"
  fi
...
show_lsp_menu() {
  ...
  while true; do
    ...
    [ "$debug" ] || tput reset
    if [ "${have_rich}" ]; then
      rich "[cyan]Webdev LSP Servers Menu[/cyan]" -p -a rounded -c -C
      rich "[b green]Enable/Disable LSP servers used by[/] [b yellow]~/.config/nvim-Webdev[/]" -p -c
    else
...
show_form_help() {
  if [ "${have_rich}" ]; then
    rich "[cyan]Webdev Formatters and Linters Menu Help[/cyan]" -p -a rounded -c -C
  else
    printf "\n\tWebdev Formatters and Linters Menu Help\n"
  fi
...
show_formlint_menu() {
  ...
  while true; do
    ...
    [ "$debug" ] || tput reset
    if [ "${have_rich}" ]; then
      rich "[cyan]Webdev Formatters and Linters Menu[/cyan]" -p -a rounded -c -C
      rich "[b green]Enable/Disable formatters and linters used by[/] [b yellow]~/.config/nvim-Webdev[/]" -p -c
    else
